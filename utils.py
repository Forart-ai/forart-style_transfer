...
def get_train_images(paths, resize_len=512, crop_height=256, crop_width=256):
    ...
    for path in paths:
        ...
        image = np.array(Image.fromarray(obj=image, mode='F').resize(size=[new_height, new_width], resample=Image.NEAREST))
...
def get_images(paths, height=None, width=None):
    ...
    for path in paths:
        ...
        if height is not None and width is not None:
            ...
            image = np.array(Image.fromarray(obj=image, mode='F').resize(size=[height, width], resample=Image.NEAREST))
